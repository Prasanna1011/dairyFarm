...
const EditCwh = () => {
  ...
  const [cwhEditData, setCwhEditData] = useState([]);
  ...
  const validation = useFormik({
    ...
    initialValues: {
      name: cwhEditData?.name || "",
      city: cwhEditData?.city_id || "",
      farm: cwhEditData?.farm_id || "",
      warehouse_manager: cwhEditData.warehouse_manager || "",
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        toast.success(`CWH Updated successfully`, {
          position: "top-center",
          autoClose: 4000,
          closeOnClick: true,
          pauseOnHover: true,
  ...
  const geEditCwhData = async () => {
    const { data } = await axios.get(`${API_CWH_VIWE_BY_ID}${id}`, config);
    setCwhEditData(data.data);
    console.log("cwhEditData", cwhEditData);
  };
  ...
  useEffect(() => {
    ...
    validation.setFieldValue("is_active", cwhEditData.is_active || false); // Set the initial value for the checkbox
  }, []);

  useEffect(() => {
    if (cwhEditData) {
      validation.setFieldValue("is_active", cwhEditData.is_active || false);
    }
  }, [cwhEditData]);
                  ...
                  <Col md="6">
                    ...
                    <Col md="6">
                      <FormGroup className="mb-3">
                        ...
                        {validation.touched.warehouse_manager && validation.errors.warehouse_manager ? (
                          <FormFeedback type="invalid">
                            {validation.errors.warehouse_manager}
                          </FormFeedback>
                        ) : null}
